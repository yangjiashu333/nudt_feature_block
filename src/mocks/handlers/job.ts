...
export const jobHandlers = [
  ...
  http.get('*/api/train/status/:jobId', async ({ params }) => {
    ...
    try {
      const { jobId } = params;
      const status = jobStatusMap.get(jobId as string) || {
        status: 'pending' as JobStatus,
        progress: 0,
      };
      ...
      return new HttpResponse(JSON.stringify({ message: 'Failed to get job status' }), {
        status: 404,
      });
    }
  }),
  ...
  http.get('*/api/train/result/:jobId', async ({ params }) => {
    ...
    if (!job || job.status !== 'done') {
      return new HttpResponse(JSON.stringify({ message: 'Training result not available' }), {
        status: 404,
      });
    }
  ...
  http.get('*/api/val/result/:jobId', async ({ params }) => {
    ...
    if (!valJob || valJob.status !== 'done') {
      return new HttpResponse(JSON.stringify({ message: 'Validation result not available' }), {
        status: 404,
      });
    }
